...
class SwapController extends Controller
{
    ...
    public function verifyOnChain(Request $request, RaceCoinService $raceCoin): JsonResponse
    {
    ...
    private function verifyBscTransaction(string $txHash, float $claimedUsd, string $fromAddress): array
    {
        ...
        if (trim($apiKey) === '') {
            return ['ok' => false, 'reason' => 'Swap verification is not configured. Contact support.'];
        }
        ...
        try {
            ...
            Log::error('BSCScan API error (swap)', ['error' => $e->getMessage(), 'tx' => $txHash]);
