...
<head>
    ...
    @if(\Orchid\Support\Locale::isRtl())
        <link rel="stylesheet" type="text/css" href="{{ \App\Support\OrchidAsset::mix('/css/orchid.rtl.css') }}" data-turbo-track="reload">
    @else
        <link rel="stylesheet" type="text/css" href="{{ \App\Support\OrchidAsset::mix('/css/orchid.css') }}" data-turbo-track="reload">
    @endif
    ...
    @foreach(collect(['/js/manifest.js', '/js/vendor.js', '/js/orchid.js']) as $key)
        <script src="{{ \App\Support\OrchidAsset::mix($key) }}" data-turbo-track="reload" type="text/javascript"></script>
    @endforeach
