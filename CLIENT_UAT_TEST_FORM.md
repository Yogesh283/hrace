...
> Never write private keys / seed phrases. Public address (`0x…`) and tx hash only.
...
For each row: **Status** + **Tx hash** (BscScan Testnet) + short **Notes**.
...
| 03 | Approve USDT | A | Approve before Stake/ICO | MetaMask success | | | |
...
| 10 | Claim ROI | A or B | Claim after **≥ 1 day** | RACE to wallet | | | |
...
**Status codes:** `PASS` · `FAIL` · `SKIP` (put reason in Notes)

---

## D) Bug report (FAIL rows only — copy paste)
...
```
(write here — what worked, what did not)
```
...
2. Get **tBNB** from faucet  
3. Get **TEST-USDT** mint from ops  
4. **A ≥ $50** → **B under A ≥ $50** → check **3% referral** on A  
